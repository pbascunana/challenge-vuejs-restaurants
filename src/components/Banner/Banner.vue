  ...
  <div class="banner" :class="bannerStyles">
    <div class="banner__wrapper">
      <div class="banner__image" v-if="backgroundMedia.type === 'image'">
        <img id="bannerImage" class="banner__image-asset" v-lazy="backgroundMedia.src" :alt="text" :title="text" />
      </div>
      <div class="banner__video" v-if="backgroundMedia.type === 'video'">
        <video id="bannerVideo" class="banner__video-asset" autoplay muted>
          <source :src="backgroundMedia.src" type="video/mp4" />
        </video>
      </div>
      <div class="banner__text" v-if="text">
        <span id="bannerText" class="banner__text-message">{{ text }}</span>
      </div>
    </div>
  </div>
</template>
